...
async def test_industry_full_system_suite():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        ...
        res_forbidden = await ac.post("/api/v1/venues", json={"name": "Illegal Screen", "address": "Block A", "city": "Test", "state": "MH", "rows": 5, "seats_per_row": 10}, headers=cust1_headers)
        ...
        prices = {c["id"]: 500.0 for c in categories} if categories else {}
        ...
        assert show_resp.status_code == 201, f"Show creation failed: {show_resp.text}"
        show_id = show_resp.json()["id"]

        # Fetch Seat Map
        seatmap_resp = await ac.get(f"/api/v1/shows/{show_id}/seats", headers=cust1_headers)
        assert seatmap_resp.status_code == 200
        seats = seatmap_resp.json()
        assert len(seats) == 20, f"Show must initialize 20 show_seats, got {len(seats)}"
        ...
        hold_req_1 = ac.post("/api/v1/holds", json={"show_id": show_id, "show_seat_ids": [seat_vip["id"]]}, headers=cust1_headers)
        hold_req_2 = ac.post("/api/v1/holds", json={"show_id": show_id, "show_seat_ids": [seat_vip["id"]]}, headers=cust2_headers)
        ...
        assert 201 in status_codes, f"One customer hold request must succeed: res1={res1.status_code}, res2={res2.status_code}"
        assert 409 in status_codes, f"Simultaneous hold attempt must fail with 409 Conflict: res1={res1.status_code}, res2={res2.status_code}"

        # Determine winner
        winner_res = res1 if res1.status_code == 201 else res2
        hold_1_data = winner_res.json()
        hold_1_id = hold_1_data["id"]
        ...
        hold_prem_resp = await ac.post("/api/v1/holds", json={"show_id": show_id, "show_seat_ids": [seat_prem["id"]]}, headers=cust1_headers)
        assert hold_prem_resp.status_code == 201
        hold_p_data = hold_prem_resp.json()

        book_p_resp = await ac.post("/api/v1/bookings", json={"show_id": show_id, "hold_id": hold_p_data["id"], "hold_token": hold_p_data["hold_token"]}, headers=cust1_headers)
